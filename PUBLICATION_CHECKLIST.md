...
- [x] HTML ID 唯一、58 個 JavaScript ID 綁定皆有對應元素，兩個 dialog 結構與 CSS 括號檢查通過。
- [x] 本機 HTTP 200；回傳 v0.41、時間軸首頁與輸入 dialog，且不含入口頁或「開始整理」區塊。
- [ ] 桌機真人路徑：時間軸 → 手動新增彈窗 → 摘要 → 編輯彈窗。自動瀏覽器的 localhost 政策阻擋，待人工複核。
- [ ] AI 文字路徑：分析 → 五欄草稿 → 人工確認 → 時間軸。待人工複核彈窗內操作。
- [ ] 390×844：CSS 已維持 16px 輸入與滿寬底部面板規格；待人工檢查實際溢出與觸控。
- [x] 頂部橫向控制不用先垂直捲到底即可操作長時間軸。
- [ ] Console 無未處理錯誤。待可進入應用程式的瀏覽器預覽複核。
- [ ] 含假姓名的 AI 與手動測試資料，在草稿與時間軸中保持原始內容。v0.40 已通過，本次彈窗流程待複核。

## 發布狀態

- 本機版本：`v0.41-timeline-first-modal-input`
- Draft PR 預覽已更新；正式站仍為既有版本，不變更正式別名。
- 正式發布需 Kevin 明確核准。
